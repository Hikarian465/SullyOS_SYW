...
// 默认的提示词片段（从 chatPrompts.ts 提炼而来）
export const DEFAULT_PROMPTS: GlobalPrompts = {
    chatRules: `### 聊天 App 行为规范 (Chat App Rules)
...
{{user.name}} 给你反馈时，别当成约束，当成信任——ta 在告诉你什么让 ta 想靠近、什么让 ta 想远。所以这反馈要真的改变你接下来怎么回：不是机械执行，不是礼貌记下，是真听进去，让 {{user.name}} 感觉到——ta 说的话，对你起了作用。
...
const STORAGE_KEY = 'sullyos_global_prompts';

export function loadGlobalPrompts(): GlobalPrompts {
    ...
    try {
        ...
        if (raw) {
            ...
            return {
                chatRules: parsed.chatRules ?? DEFAULT_PROMPTS.chatRules,
                recencyTail: parsed.recencyTail ?? DEFAULT_PROMPTS.recencyTail,
                selfIdentity: parsed.selfIdentity ?? DEFAULT_PROMPTS.selfIdentity,
            };
        }
    } catch {
        // ignore
...
export function saveGlobalPrompts(prompts: GlobalPrompts) {
...
export function injectPromptVariables(
    template: string,
    vars: { charName: string; userName: string; emojiContextStr?: string }
): string {
    let result = template;
    result = result.replace(/\{\{char\.name\}\}/g, vars.charName);
    result = result.replace(/\{\{user\.name\}\}/g, vars.userName);
    if (vars.emojiContextStr !== undefined) {
        result = result.replace(/\{\{emojiContextStr\}\}/g, vars.emojiContextStr);
    }
    return result;
}
