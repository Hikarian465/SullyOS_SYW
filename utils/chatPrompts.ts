...
但务必记住：这只是 ta 挂在虚拟空间里的一个化身状态（类似游戏挂机 / AFK），**并不代表 ${uname} 本人此刻真守在游戏里**——ta 很可能早已离开屏幕、正在现实里忙别的或休息。所以别据此认定"ta 正盯着你""ta 现实里也在干这件事"，也别把它当成 ta 在跟你说话。你和 ta 的真实关系、近况一律以你们的聊天记录为准；这条只是彼方这个虚拟空间里的一个在场提示而已。\n`;
        ...
        const globalPrompts = (await import('./promptManager')).loadGlobalPrompts();
        ...
        const { injectPromptVariables } = await import('./promptManager');
        baseSystemPrompt += injectPromptVariables(globalPrompts.chatRules, {
            charName: char.name,
...
${voiceActingGuide()}`;
        ...
        let recencyTail = '\n\n' + injectPromptVariables(globalPrompts.recencyTail, {
            charName: char.name,
            userName: userProfile.name,
        });

        // 末尾 recency 第二棒：把「你是谁」拉到整段 prompt 的最后一句，焊成钢印。
        recencyTail += '\n\n' + injectPromptVariables(globalPrompts.selfIdentity, {
            charName: char.name,
            userName: userProfile.name,
        });
