...
const buildLegacyStyleProactiveHint = (targetName: string, includeTime: boolean) => {
  ...
  return [
    ...
    `正文尽量短，通常拆成 2 到 3 个短气泡，每个气泡之间用真实换行；只有一句话明显更自然时才只发一个。如果 ${target} 很久没来找你，可以轻轻带一点想念、好奇或者小小抱怨。`,
  ].join('\n');
};
...
export const buildFirePack = async (
  ...
  const template = [
    ...
    '- 像真实聊天一样简短自然，通常写成 2 到 3 个短聊天气泡；只有一句话明显更自然时才只写一个。',
    '- 每个聊天气泡之间必须使用真实换行，不要用斜杠 n、编号或空格假装分隔；不要写时间戳、名字前缀、系统提示。',
    '- 不要出现“作为AI”“系统提示”等元话语。',
